...
def window_search(binary_warped,img):
    ...
    if len(leftx) > 0 and len(lefty) > 0:
        ...
        problem_img = binary_warped*255
        img= cv2.cvtColor(img,cv2.COLOR_RGB2BGR)
        cv2.imwrite(f"{output_dir}/window_problem{save_count:03d}.jpg", img)
        print(f'Saved window_problem frame{save_count:03d}.')
        save_count += 1
...
def margin_search(binary_warped,img):
    ...
    if len(leftx) > 0 and len(lefty) > 0:
        ...
        problem_img = binary_warped*255
        img= cv2.cvtColor(img,cv2.COLOR_RGB2BGR)
        cv2.imwrite(f"{output_dir}/margin_problem{save_count:03d}.jpg", img)
        print(f'Saved margin_problem frame{save_count:03d}.')
        save_count += 1
    ...
    if len(rightx) > 0 and len(righty) > 0:                                                
        ...
        problem_img = binary_warped*255
        img= cv2.cvtColor(img,cv2.COLOR_RGB2BGR)
        cv2.imwrite(f"{output_dir}/problem{save_count:03d}.jpg", img)
        print(f'Saved margin_problem frame{save_count:03d}.')
        save_count += 1
...
def find_lanes(combined,img):
    if left_line.detected and right_line.detected:  # Perform margin search if exists prior success.
        ...
        validate_lane_update(combined, left_lane_inds, right_lane_inds)
...
def process_img(img):
    ...
    '''
    cv2.imshow("scaled_s_binary", scaled_s_bin)
    cv2.waitKey(0)
    cv2.destroyAllWindows()    
    '''
    ...
    # Find Lanes
    output_img = find_lanes(combined,img)
