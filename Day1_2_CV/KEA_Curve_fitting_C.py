...
def hsv_thresh(img, thresh_min=0, thresh_max=255):
    # Convert to HLS color space and separate the S channel
...
def hsv_rgb_lane_detection(img):
    ...
    hsv = cv2.cvtColor(img, cv2.COLOR_RGB2HSV)
    '''
    # Define range for lane line colors in HSV
    lower_yellow = np.array([20, 100, 100])
    upper_yellow = np.array([30, 255, 255])
    lower_white = np.array([0, 0, 200])
    upper_white = np.array([255, 30, 255])
    lower_yellow = np.array([20, 22, 100])
    upper_yellow = np.array([135, 90, 255])
    lower_white = np.array([0, 0, 200])
    upper_white = np.array([255, 30, 255]) 
    '''
...
def window_search(binary_warped):
    ...
    if len(leftx) > 0 and len(lefty) > 0:
        ...
        left_fit = np.array([0, 0, 0])  # Use a default value if no previous value is available
    ...
    #cv2.imshow("window_search", out_img) 
    #cv2.waitKey(0)
...
def margin_search(binary_warped):
    ...
    '''
    if len(leftx) > 0:
        leftx_values.append(leftx)
        left_fit = np.polyfit(lefty, leftx, 2)
    else:
        for previous_leftx in reversed(leftx_values):
            if len(previous_leftx) > 0 and len(lefty) > 0:
                left_fit = np.polyfit(lefty, previous_leftx, 2)
                break
        else:
            left_fit = np.array([0, 0, 0])  # Use a default value if no previous value is available

    if len(rightx) > 0:
        rightx_values.append(rightx)
        right_fit = np.polyfit(righty, rightx, 2)
    else:
        for previous_rightx in reversed(rightx_values):
            if len(previous_rightx) > 0 and len(righty) > 0:
                right_fit = np.polyfit(righty, previous_rightx, 2)
                break
        else:
            right_fit = np.array([0, 0, 0])   # Use a default value if no previous value is available
    '''    
    # Fit a second order polynomial to each
    #left_fit = np.polyfit(lefty, leftx, 2)
    #right_fit = np.polyfit(righty, rightx, 2)
...
def validate_lane_update(img, left_lane_inds, right_lane_inds):
    ...
    if len(left_line_allx) <= 100 or len(right_line_allx) <= 100:
    ...
    if  lane_width < 300 or lane_width > 550:#800
        left_line.detected = False
        right_line.detected = False
        return 
...
def process_img(img):
    ...
    N_combined = N_binary / 255  # Scale N_binary to [0, 1]
    cv2.imshow("N_binary", N_binary)
    ...
    # Find Lanes
    output_img = find_lanes(N_combined)
...
# Initialize deques to store the last n values
leftx_values = deque(maxlen=5)  # Change n to the number of values you want to remember
rightx_values = deque(maxlen=5)  # Change n to the number of values you want to remember

# Load video

cap = cv2.VideoCapture('Python_code/KEA_0523/test_img/video/challenge_video.mp4')
print("Loading video...")
if not cap.isOpened():
