        ...
        if frame is not None:
            ...
            if(detect_num > 0) :
                ...
                for j in range(len(track_bbs_ids.tolist())):
                    ...
                    #print("name_idx {}".format(name_idx))
                    ...
                    if ID_Num not in paths:
                        paths[ID_Num] = deque(maxlen=max_path_length)

                    clabel = class_to_label(name_idx)
                    #print("name_idx {} clabel {}".format(name_idx, clabel))
                    color = ID_to_color(ID_Num) #class_colors[ID_Num]
                    
                    paths[ID_Num].append(CenterPoint[0])
                    for i in range(1, len(paths[ID_Num])):
                       cv2.line(frame, tuple(map(int, paths[ID_Num][i-1]))
                        ,tuple(map(int, paths[ID_Num][i]))
                        ,color, 2)
